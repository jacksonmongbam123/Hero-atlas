...
export default function UserProfileCard({ user, onLogout }: UserProfileCardProps) {
  ...
  return (
    <div className="w-full bg-slate-900/30 border border-slate-800/60 rounded-3xl p-5 shadow-lg relative overflow-hidden backdrop-blur-sm select-none mt-6">
      ...
      <button
        onClick={onLogout}
        className="w-full bg-white/5 hover:bg-white/10 text-red-500 hover:text-red-400 font-medium py-2.5 px-4 rounded-lg text-xs flex items-center justify-center gap-2 border border-white/10 transition-all cursor-pointer"
      >
