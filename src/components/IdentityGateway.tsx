...
export default function IdentityGateway({ theme, onToggleTheme, onSuccess }: IdentityGatewayProps) {
    ...
    try {
      ...
      for (const url of loginUrls) {
        try {
          ...
          if (res.ok && body && (body.user || body.token || body.status === "success")) {
            response = res;
            data = body;
            break;
          }
      ...
      if (!response || !response.ok) {
        const msg = data.message || `Authorization failed. Please check your credentials and selected role.`;
        setErrorMsg(msg);
  ...
  return (
    <div className={`min-h-screen bg-portal-main text-slate-100 flex flex-col justify-between p-4 sm:p-6 font-sans selection:bg-indigo-500/30 transition-colors duration-200 ${isMobileApp ? "pb-[30px]" : ""}`}>
      ...
      <div
        className="max-w-md w-full mx-auto flex justify-between items-center py-2"
        style={{
